...
namespace UnrealLua::HashUtility
{
	...
	constexpr uint32 CRCTablesSB8[8][256] = 
	{
		{
	...
	*/
	...
	static constexpr uint32 ToUnsigned(const CharType Char)
	{
		return (typename TUnsignedIntType<sizeof(CharType)>::Type)Char;
	}
	
	template <typename CharType>
	static constexpr CharType ToUpper(const CharType Char)
	{
		return (CharType)(ToUnsigned(Char) - ((uint32(Char) - 'a' < 26u) << 5));
	}
	
	template <typename CharType>
	static constexpr typename TEnableIf<sizeof(CharType) == 1, uint32>::Type StrCrc32(const CharType* Data, uint32 CRC = 0)
	{
		/* Overload for when CharType is a byte, which causes warnings when right-shifting by 8 */
		CRC = ~CRC;
		while (const CharType Ch = *Data++)
		{
			const CharType Chr = ToUpper<ANSICHAR>(Ch);
			CRC = (CRC >> 8) ^ CRCTablesSB8[0][(CRC ^ Chr) & 0xFF];
	...
	static constexpr uint32 StrCrc32(const TCHAR* Data, uint32 CRC = 0)
	{
		/* Overload for when CharType is a byte, which causes warnings when right-shifting by 8 */
		CRC = ~CRC;
		while (const TCHAR Ch = *Data++)
		{
			ANSICHAR Ac = CharCast<ANSICHAR>(Ch);
			Ac = ToUpper<ANSICHAR>(Ac);
			CRC = (CRC >> 8) ^ CRCTablesSB8[0][(CRC ^ Ac) & 0xFF];
			CRC = (CRC >> 8) ^ CRCTablesSB8[0][(CRC     ) & 0xFF];
			CRC = (CRC >> 8) ^ CRCTablesSB8[0][(CRC     ) & 0xFF];
			CRC = (CRC >> 8) ^ CRCTablesSB8[0][(CRC     ) & 0xFF];
		}
	...
	static uint32 StrCrc32(const FName& name)
	{
		const FNameEntry* entry = name.GetDisplayNameEntry();
		static TCHAR buffer[NAME_SIZE];
		entry->GetName(buffer);
		return StrCrc32(buffer);
	}

	inline uint64 Update64(uint64 InitialCrc, const void* Data, int64 NumBytes)
	{
	...
	inline uint64 Calc64(const void* Data, int64 NumBytes)
	{
		return ~Update64(uint64(~(0LLU)), Data, NumBytes);
	}

	template <typename CharType>
	static typename TEnableIf<sizeof(CharType) == 1, uint64>::Type StrCrc64(const CharType* Data, uint32 CRC = 0)
	{
		int32 len = strlen(Data);
		return Calc64(Data, len);
	}
	
	static uint64 StrCrc64(const TCHAR* Data, uint32 CRC = 0)
	{
		auto casted = StringCast<char>(Data);
		return Calc64(casted.Get(), casted.Length());
	}

	ELuaValueType GetLuaValueType(sol::object value);

	void PrintLuaValue(sol::object value, FString msg);
}
