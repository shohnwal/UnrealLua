...
template <typename T>
constexpr bool TIsLuaVariant_V = false;

template <typename... Ts>
constexpr bool TIsLuaVariant_V<TLuaVariant<Ts...>> = true;

template <typename T> constexpr bool TIsLuaVariant_V<const          T> = TIsLuaVariant_V<T>;
template <typename T> constexpr bool TIsLuaVariant_V<      volatile T> = TIsLuaVariant_V<T>;
template <typename T> constexpr bool TIsLuaVariant_V<const volatile T> = TIsLuaVariant_V<T>;

/** Determine the number of types in a TLuaVariant */
template <typename>
constexpr SIZE_T TLuaVariantSize_V = 0;

template <typename... Ts>
constexpr SIZE_T TLuaVariantSize_V<TLuaVariant<Ts...>> = sizeof...(Ts);

template <typename T> constexpr SIZE_T TLuaVariantSize_V<const          T> = TLuaVariantSize_V<T>;
template <typename T> constexpr SIZE_T TLuaVariantSize_V<      volatile T> = TLuaVariantSize_V<T>;
template <typename T> constexpr SIZE_T TLuaVariantSize_V<const volatile T> = TLuaVariantSize_V<T>;

template <typename T>
struct UE_DEPRECATED(5.4, "TIsVariant<T> has been deprecated, please use TIsVariant_V<std::remove_reference_t<T>> instead") TIsLuaVariant
{
	static constexpr inline bool Value = TIsLuaVariant_V<T>;
};

/** Determine the number of types in a TLuaVariant */
template <typename> struct TLuaVariantSize;

template <typename T>
struct UE_DEPRECATED(5.4, "TLuaVariantSize<T> has been deprecated, please use TLuaVariantSize_V<std::remove_reference_t<T>> instead") TLuaVariantSize
{
	static constexpr inline SIZE_T Value = TLuaVariantSize_V<std::remove_reference_t<T>>;
};
...
namespace Variant
{
	/** A shim to get at FArchive through a dependent name, allowing TLuaVariant.h to not include Archive.h. Only calling code that needs serialization has to include it. */
	template <typename T>
	...
	struct TLuaVariantStorage
	{
		...
		const auto& GetValueAsIndexedType() const&
		{
			...
			return const_cast<TLuaVariantStorage*>(this)->template GetValueAsIndexedType<N>();
		}
	...
	/** A utility for loading a specific type from FArchive into a TLuaVariant */
	template <typename T, typename VariantType>
	struct TLuaVariantLoadFromArchiveCaller
	{
	...
	template <typename... Ts>
	struct TLuaVariantLoadFromArchiveLookup
	{
		using VariantType = TLuaVariant<Ts...>;
		static_assert((std::is_default_constructible_v<Ts> && ...), "Each type in TLuaVariant template parameter pack must be default constructible in order to use FArchive serialization");
		static_assert((TModels_V<CInsertable<FArchive&>, Ts> && ...), "Each type in TLuaVariant template parameter pack must be able to use operator<< with an FArchive");

		/** Load the type at the specified index from the FArchive and emplace it into the TLuaVariant */
		static void Load(SIZE_T TypeIndex, FArchive& Ar, VariantType& OutVariant)
		{
			static constexpr void(*Loaders[])(FArchive&, VariantType&) = { &TLuaVariantLoadFromArchiveCaller<Ts, VariantType>::Load... };
	...
	{
		return First.GetIndex() + TLuaVariantSize_V<Variant0> * EncodeIndices(Rest...);
	}
	...
	/** Cast a TLuaVariant to its private base */
	template <typename... Ts>
	TLuaVariantStorage<Ts...>& CastToStorage(TLuaVariant<Ts...>& Variant)
	{
		return *(TLuaVariantStorage<Ts...>*)(&Variant);
	}

	template <typename... Ts>
	TLuaVariantStorage<Ts...>&& CastToStorage(TLuaVariant<Ts...>&& Variant)
	{
		return (TLuaVariantStorage<Ts...>&&)(*(TLuaVariantStorage<Ts...>*)(&Variant));
	}

	template <typename... Ts>
	const TLuaVariantStorage<Ts...>& CastToStorage(const TLuaVariant<Ts...>& Variant)
	{
		return *(const TLuaVariantStorage<Ts...>*)(&Variant);
	}
	...
	{
		constexpr SIZE_T VariantSizes[] = { TLuaVariantSize_V<std::decay_t<Variants>>... };
...
#if !UE_TVARIANT_TRIVIAL_DESTRUCTOR_USING_CONCEPTS
	template <typename T, typename... Ts>
	struct TDestructibleVariantStorage : TLuaVariantStorage<T, Ts...>
	{
	protected:
		~TDestructibleVariantStorage()
		{
			UnrealLua::Variant::TDestructorLookup<T, Ts...>::Destruct(this->TypeIndex, this);
		}
	};
#endif
